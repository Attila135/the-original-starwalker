return function(cutscene, event, player, facing)
    ...
    if Game.money <= 0 and not event.interacted then
        ...
        cutscene:text("*** Lmao you're broke as\n          [func:remove][color:yellow]fuck[noskip][wait:1.2s]", nil,
            event, {
                functions = {
                    remove = function()
                        ...
                        if vessel then
                            ...
                            vessel.sprite.flip_x = true
                        end
                        if noelle then
                            noelle:setSprite("collapsed")
                        end
                        if watercooler then
                            watercooler:setSprite("fell")
                            watercooler.sprite.flip_x = true
                        end
                    end
                }
            })
        ...
        cutscene:wait(0.5)
        if watercooler then
            watercooler.sprite.flip_x = false
            watercooler:resetSprite()
            watercooler:shake(4)
    ...
    if not event.interacted then
        ...
        cutscene:text(
            "*** I'm the original   [color:yellow]Starwalker[color:reset][talk:false][react:1][wait:5][react:2][wait:5][react:3][wait:5][react:sussy]",
            {
                reactions = {
        ...
        if susie then
            ...
            cutscene:text("*** [color:yellow]You[color:reset] are [color:yellow]Pissing[color:reset] me off...", nil,
                event)
