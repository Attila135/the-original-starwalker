...
function masktutwave:update()
    ...
    if (Game.battle ~= nil and Game.battle.party[1].chara.health < 40 and self.canBetter == true) then
        Assets.playSound("wolf_bthan"); self.canBetter = false;
    end
    if (Game.battle ~= nil and Game.battle.party[1].chara.health < 30 and self.canBetter == false) then
        Game.battle.party[1]:heal(5000); self.canBetter = true;
    end
