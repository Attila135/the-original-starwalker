local dustpawclaw_1, super = Class(Wave)
local angleChange = 1
local timeVar = 0
local initialX = 0
local initialY = 0
...
-- function dustpawclaw_1:onDamage(soul)
-- super.onDamage(self)
...
function dustpawclaw_1:onStart()
    ...
    self.pawAttack = 2;
    self.pawReverse = Utils.pick({-1, 1});
    -- attacker2:StartSwirl()
    end
    
end

function dustpawclaw_1:update()
    -- Code here gets called every frame
    -- self.timer = self.timer + (1 * DTMULT)
    self.timeVar = self.timeVar + 1
    super.update(self)
   -- if (self.timeVar == 3 or self.timeVar == 6 or self.timeVar == 9 or self.timeVar == 110 or self.timeVar == 113 or self.timeVar == 116)
   --     then
   --     self.pawAttack = self.pawAttack - 1;
   --     self.randomPawX = 1 * 160
   --     self.randomPawY = Game.battle.soul.y
   --     self:spawnBullet("pawclaw", Game.battle.arena.x + self.randomPawX, self.randomPawY, Utils.angle(Game.battle.soul.x + self.randomPawX, self.randomPawY, Game.battle.soul.x, Game.battle.soul.y), 1.2, Game.battle.soul.x, Game.battle.soul.y, 2 * 0)
   --     
   --
   --     self.randomPawX = -1 * 160
   --     self.randomPawY = Game.battle.soul.y
   --     self:spawnBullet("pawclaw", Game.battle.arena.x + self.randomPawX, self.randomPawY, Utils.angle(Game.battle.soul.x + self.randomPawX, self.randomPawY, Game.battle.soul.x, Game.battle.soul.y), 1.2, Game.battle.soul.x, Game.battle.soul.y, 2 * 0)
   --     -- self.pawReverse = self.pawReverse * -1;
   --     end
    ...
    if (1 == 1) 
        ...
        for _, attacker in ipairs(attackers) do
            ...
            if (self.attackAmount > 0)
                ...
                self:spawnBullet("dustpawclaw", Game.battle.arena.x + self.randomPawX, Game.battle.arena.y + self.randomPawY, Utils.angle(Game.battle.arena.x + self.randomPawX, Game.battle.arena.y + self.randomPawY, Game.battle.arena.x, Game.battle.arena.y), 1.2, Game.battle.arena.x, Game.battle.arena.y, self.spinSpeeder)
