local tutorialtip, super = Class(Bullet)

function tutorialtip:onDamage(soul)
if (self.damage ~= 0)
    then
    super.onDamage(self, soul) 
    --self.damage = 0;
    self.tp = 0;
    end

end

function tutorialtip:onCollide(soul)
super.onCollide(self, soul)
if (self.damage ~= 0)
    then
    --self.damage = 0;
    self.tp = 0;
    end

end

function tutorialtip:init(x, y, dir, speed)
    -- Last argument = sprite path
    super.init(self, x, y, "tutorialtext")

    --self.threadSprite = Sprite("ralseithreads"); 
    --self:addChild(self.threadSprite);
    --Kristal.Console:log("T");

    -- Move the bullet in dir radians (0 = right, pi = left, clockwise rotation)
    self.physics.direction = dir
    -- Speed the bullet moves (pixels per frame at 30FPS)
    self.physics.speed = speed

    self.destroy_on_hit = false;
    self.remove_offscreen = false;
    self.alwaysBreak = true;
    self.damage = 10;
    self.tp = 0;
    self:setScale(1, 1);
    self.alpha = 0;
    self.timer = Timer();
    self:addChild(self.timer);
    --self.timer:tween(1.0, self, {collider.radius = 100}, 'out-quad');
    self.timer:tween(3.5, self, {alpha = 1}, 'in-quad');
    --self.inv_timer = 0.2;

end

function tutorialtip:remove()
    super.remove(self)
end

function tutorialtip:draw()
    super.draw(self)
end

function tutorialtip:update()
    -- For more complicated bullet behaviours, code here gets called every update
    --local lineX, lineY = self:getScreenPos();
    --if (lineX < Game.battle.arena.x - Game.battle.arena.width / 2)
    --    then
    --    self.wave:spawnBullet("breakexplosion", lineX, lineY);
    --    self:remove();
    --    end

    super.update(self)
end

return tutorialtip