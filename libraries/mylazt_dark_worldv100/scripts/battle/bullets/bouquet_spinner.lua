local bouquet_spinner, super = Class(Bullet)

function bouquet_spinner:init(x, y, angleStart, angleSpeed, angleDistance, shootMoment)
    -- Last argument = sprite path
    super.init(self, x, y, "bullets/bouquet_spinner")
    ...
    self.timeVar = 0;
    self.currentAngle = 0 + angleStart;
    self.alpha = 0;
...
function bouquet_spinner:update()
    -- For more complicated bullet behaviours, code here gets called every update
    if (self.alpha < 1) then self.alpha = self.alpha + 0.05; end
    self.timeVar = self.timeVar + 1;
    
    if (self:collidesWith(Game.battle.arena.collider))
        then
        Assets.playSound("darkbomb")
        self.wave:spawnBullet("flower_spinnercore", self.x, self.y, self.currentAngle + math.rad(180), 4);
        self:remove();
        end

    if (self.timeVar < self.shootMoment)
        then
        self.currentAngle = self.currentAngle + self.angleSpeed;
        self.rotation = math.cos(self.timeVar / 2) * math.rad(15);

        else 
        -- begin swinging/charging
        self.backVar = self.backVar + 1;
        if (self.wentBack == false)
            ...
            -- shoot forward
            if (self.backVar % 2 == 0) then self.rotation = self.rotation + math.rad(90); end
