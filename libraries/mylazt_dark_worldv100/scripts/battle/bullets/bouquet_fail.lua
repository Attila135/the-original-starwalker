local bouquet_fail, super = Class(Bullet)

function bouquet_fail:init(x, y, angleStart, angleSpeed, angleDistance, shootMoment)
    -- Last argument = sprite path
    super.init(self, x, y, Utils.pick({"bullets/bouquet_spinner", "bullets/bouquet_aimline", "bullets/bouquet_regular"}))
...
function bouquet_fail:update()
    ...
    if (self:collidesWith(Game.battle.arena.collider))
        ...
        self.wave:spawnBullet("flower_fadefast", self.x, self.y, self.currentAngle + math.rad(0), 6);
        self.wave:spawnBullet("flower_fadefast", self.x, self.y, self.currentAngle + math.rad(0 + 25), 6);
        self.wave:spawnBullet("flower_fadefast", self.x, self.y, self.currentAngle + math.rad(0 - 25), 6);
        self:remove();
        end

    if (self.wave.timeVar < self.shootMoment)
        then
        ...
        if (self.wentBack == false)
            then
            self.angleDistance = Utils.ease(self.originalDistance, self.originalDistance + self.backDistance, self.backVar / 45, "in-out-quad")
