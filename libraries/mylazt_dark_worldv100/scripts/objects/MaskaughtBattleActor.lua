local MaskaughtBattleActor, super = Class(ActorSprite)

function MaskaughtBattleActor:init(actor)
    ...
    self.maskaughtHead = self:SpawnPart("enemies/MaskaughtBattle/head", "head"); 
    self.maskaughtBody = self:SpawnPart("enemies/MaskaughtBattle/body", "body"); 
 
    self.schoolNote = "'My right is actually your left. Isn't that confusing?'"

    self.maskaughtArmleft = self:SpawnPart("enemies/MaskaughtBattle/armhigh", "armfront"); 
    self.maskaughtArmright = self:SpawnPart("enemies/MaskaughtBattle/armcheek", "armback"); 

    self.maskaughtTail = self:SpawnPart("enemies/MaskaughtBattle/tail", "tail"); 

    

    self.maskaughtLegleft = self:SpawnPart("enemies/MaskaughtBattle/legfront", "legfront"); 
    self.maskaughtLegright = self:SpawnPart("enemies/MaskaughtBattle/legback", "legback"); 
...
for _, mPart in ipairs(self.maskaughtParts)
    do
    self.animTime = self.animTime + 1; -- this is still running
    if (mPart.moveX < 0)
        then
        if (mPart.moveXTracker >= -80)
            ...
            if (mPart.isHurting == true)
                ...
                mPart.attachTime = mPart.attachTime + 1;
    ...
    if (self.detachCount > 0)
        ...
        if (mPart.partLabel == "head")
            then
            mPart:setSprite("enemies/MaskaughtBattle/headhurt", 0);
            else
    ...
    if (self.maskaughtAnimState == "IDLE")
        ...
        if (mPart.partLabel == "body")
            ...
            self.maskaughtHead.y = self.maskaughtHead.startY + (math.sin(mPart.rotation) * -2);
            end

        if (mPart.partLabel == "armfront")
            then
            mPart.rotation = math.rad(15) * math.sin(self.animTime / 70);
            if (self.raiseTime == 0 and self.maskaughtArmleft.startSprite ~= "enemies/MaskaughtBattle/armraise_3") then self:PartSet(self.maskaughtArmleft, "enemies/MaskaughtBattle/armraise_1"); end
            if (self.raiseTime > 0 and self.raiseDuration > 0)
                then -- for lobera attacks
                self.raiseTime = self.raiseTime - 1;
                if (self.raiseTime <= self.raiseDuration) then self:PartSet(self.maskaughtArmleft, "enemies/MaskaughtBattle/armraise_1"); end
                if (self.raiseTime <= 3) then self:PartSet(self.maskaughtArmleft, "enemies/MaskaughtBattle/armraise_2"); end
                if (self.raiseTime == 0) then self:PartSet(self.maskaughtArmleft, "enemies/MaskaughtBattle/armraise_3"); end
                else -- if not in lobera attack
                if (self.raiseDuration <= 0) then self:PartSet(self.maskaughtArmleft, "enemies/MaskaughtBattle/armhigh"); end
                end
            end
        ...
        if (mPart.partLabel == "armback")
            ...
            if (self.raiseDuration > 0)
                then
                self:PartSet(self.maskaughtArmright, "enemies/MaskaughtBattle/armhide");
                else
                self:PartSet(self.maskaughtArmright, "enemies/MaskaughtBattle/armcheek");
                end
            end

        if (mPart.partLabel == "tail")
            then
            mPart.rotation = math.rad(20) * math.sin(self.animTime / 50);
        ...
        if (mPart.partLabel == "legfront")
            ...
            if (1 == 1)
                then
                if (-1 < legCos and legCos < 0.5) then mPart:setSprite("enemies/MaskaughtBattle/legnormal", 0, false); end
                if (0 < legCos and legCos < 0.5) then mPart:setSprite("enemies/MaskaughtBattle/legback", 0, false); end
                if (0.5 < legCos and legCos < 1) then mPart:setSprite("enemies/MaskaughtBattle/legbacker", 0, false); end
                end
            end
        if (mPart.partLabel == "legback")
            ...
            if (1 == 1)
                ...
                if (-1 < legCos2 and legCos2 < 0.5) then mPart:setSprite("enemies/MaskaughtBattle/legnormal", 0, false); end
                if (0 < legCos2 and legCos2 < 0.5) then mPart:setSprite("enemies/MaskaughtBattle/legback", 0, false); end
                if (0.5 < legCos2 and legCos2 < 1) then mPart:setSprite("enemies/MaskaughtBattle/legbacker", 0, false); end
                end
            end
...
function MaskaughtBattleActor:draw()
...
function MaskaughtBattleActor:ResetPart(thePart)
...
function MaskaughtBattleActor:SetState(stateName)
    ...
    if (stateName == "HURT")
        ...
        if (willDetach == true and hasDetached == false)
            then
            hasDetached = true;
            mPart.moveX = love.math.random(-6, -6); -- this is getting run
            mPart.moveY = love.math.random(0.5, 3.0) * Utils.randomSign();
            mPart:setRotationOrigin(0.5, 0.5);
            mPart:setOrigin(0.5, 0.5);
            mPart.moveRotation = math.rad(love.math.random(12, 16) * Utils.randomSign());
            mPart.isHurting = true;
            self.detachCount = self.detachCount + 1;
            end
        -- mPart.y = mPart.y + 5;
        -- Kristal.Console:log("nail")
        end
    end
    -- Kristal.Console:log("after state " .. self.maskaughtAnimState);
end
function MaskaughtBattleActor:PartInfo(partSprite)

 --   self.maskaughtHead = self:SpawnPart("enemies/MaskaughtBattle/head", "head", 0, 0, 0.5, 0.5, 30, nil); 
 --   self.maskaughtBody = self:SpawnPart("enemies/MaskaughtBattle/body", "body", 0, 15, 0.5, 0.5, 28, nil); 
 --
 --   self.maskaughtArmleft = self:SpawnPart("enemies/MaskaughtBattle/armhigh", "armfront", -4,  -4, 1.0, 0.75, 24, self.maskaughtBody); 
 --   self.maskaughtArmright = self:SpawnPart("enemies/MaskaughtBattle/armcheek", "armback", 8, 0, 1.0, 0.75, 34, self.maskaughtBody); 
 --
 --   self.maskaughtTail = self:SpawnPart("enemies/MaskaughtBattle/tail", "tail", 4, 6, 0.0, 0.5, 20, self.maskaughtBody); 
 --
 --
 --   self.maskaughtLegleft = self:SpawnPart("enemies/MaskaughtBattle/legfront", "legfront", -2, 7, 0.5, 0, 2, self.maskaughtBody); 
 --   self.maskaughtLegright = self:SpawnPart("enemies/MaskaughtBattle/legback", "legback", -2, 7, 0.5, 0, 4, self.maskaughtBody); 

    local returnValue = {addX = 0, addY = 0, originX = 0, originY = 0, addLayer = 0, connectTo = nil};

    if (partSprite == "enemies/MaskaughtBattle/head") then returnValue = {addX = 0, addY = 0, originX = 0.5, originY = 0.5, addLayer = 30, connectTo = nil}; end
    if (partSprite == "enemies/MaskaughtBattle/body") then returnValue = {addX = 0, addY = 15, originX = 0.5, originY = 0.5, addLayer = 28, connectTo = nil}; end

    if (partSprite == "enemies/MaskaughtBattle/armhigh") then returnValue = {addX = -4, addY = -4, originX = 1.0, originY = 0.75, addLayer = 24, connectTo = self.maskaughtBody}; end

    if (partSprite == "enemies/MaskaughtBattle/armraise_1") then returnValue = {addX = -4, addY = -4, originX = 0.5, originY = 0.7, addLayer = 60, connectTo = self.maskaughtBody}; end
    if (partSprite == "enemies/MaskaughtBattle/armraise_2") then returnValue = {addX = -4, addY = -4, originX = 0.5, originY = 0.7, addLayer = 60, connectTo = self.maskaughtBody}; end
    if (partSprite == "enemies/MaskaughtBattle/armraise_3") then returnValue = {addX = -4, addY = -4, originX = 0.5, originY = 0.7, addLayer = 24, connectTo = self.maskaughtBody}; end


    if (partSprite == "enemies/MaskaughtBattle/armcheek") then returnValue = {addX = 10, addY = 0, originX = 1.0, originY = 0.75, addLayer = 34, connectTo = self.maskaughtBody}; end

    if (partSprite == "enemies/MaskaughtBattle/armhide") then returnValue = {addX = 8, addY = 0, originX = 1.0, originY = 0.75, addLayer = 10, connectTo = self.maskaughtBody}; end

    if (partSprite == "enemies/MaskaughtBattle/tail") then returnValue = {addX = 4, addY = 6, originX = 0.0, originY = 0.5, addLayer = 20, connectTo = self.maskaughtBody}; end

    if (partSprite == "enemies/MaskaughtBattle/legfront") then returnValue = {addX = -2, addY = 7, originX = 0.5, originY = 0.0, addLayer = 2, connectTo = self.maskaughtBody}; end
    if (partSprite == "enemies/MaskaughtBattle/legback") then returnValue = {addX = -2, addY = 7, originX = 0.5, originY = 0, addLayer = 4, connectTo = self.maskaughtBody}; end


    return returnValue
end
function MaskaughtBattleActor:PartSet(thePart, spritePath)
...
function MaskaughtBattleActor:SpawnPart(spritePath, theLabel)
